...
function update_kit($selected_kit, $component_id)
{
	...
	if ($component_id == -1) { // new component in alias/kit 
		...
		display_notification($selected_kit.'-_-'.get_post('component'));
	...
	//display_error($component_id .'_-_'. $selected_kit .'_-_'. get_post('component'));
	$Mode = 'RESET';
...
if ($Mode == 'Delete')  // Sales kit individual component delete.
{
	// Before removing last component from selected kit check 
	// if selected kit is not included in any other kit. 
	// 	display_error(_("The component item has been deleted from this bom"));
